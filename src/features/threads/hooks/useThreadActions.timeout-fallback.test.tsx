...
describe("useThreadActions sidebar listing timeout fallback", () => {
  ...
  it(
    "case 1: claude listing timeout still keeps last-good claude entries when codex returns a session",
    async () => {
  ...
  it(
    "case 2: consecutive claude timeouts do not progressively drop more claude sessions",
    async () => {
      vi.mocked(listClaudeSessions).mockImplementation(NEVER_RESOLVES);
      vi.mocked(listWorkspaceSessions).mockResolvedValue({
        data: [
      ...
      const secondClaudeIds = secondDispatch!.threads
        .filter((t: any) => t.engineSource === "claude")
        .map((t: any) => t.id);
      expect(secondClaudeIds.length).toBeGreaterThanOrEqual(firstClaudeIds.length);
  ...
  it(
    "case 3: partial-source diagnostic remains observable after timeout-driven fallback",
    async () => {
      vi.mocked(listClaudeSessions).mockImplementation(NEVER_RESOLVES);
  ...
  it(
    "case 4: archived last-good claude entries are not resurrected by seed",
    async () => {
      vi.mocked(listClaudeSessions).mockImplementation(NEVER_RESOLVES);
      vi.mocked(listWorkspaceSessions).mockResolvedValue({
        data: [
